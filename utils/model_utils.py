...
def count_parameters(model):
    '''
    Prints the number of parameters of the given model.
    '''
    table = []
    total_params = 0
    for name, parameter in model.named_parameters():
        ...
        total_params += param
    print(tabulate(table, headers=["Modules", "Parameters"]))
...
def find_best_val(paths):
    ...
    train_results_path = get_closest_file(
        paths[0], file_names=["train_result.csv"])
...
def get_closest_file(file_path, file_names):
    ...
    raise ValueError(
        f"No files called like {file_names} found in path {file_path}")

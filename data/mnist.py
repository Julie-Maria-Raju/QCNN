...
class ExperimentDataset:
    ...
    def __init__(self, seed=0, path='./mnist', save_preload=True, logger=None, writer=None):
...
class QuantumEncodedMNIST(MNISTDataset):
    ...
    def __init__(self, params, q_device, train_size=3000, valid_size=300, test_size=300, seed=None, normalize=False,
        ...
        print("save_dir preencoded data", self.save_dir)
        self.set_save_paths()
    ...
    def preload_encoded_MNIST(self):
        ...
        if self.get_preloaded_train() is None or self.get_preloaded_test() is None:
            print("train_x_path", self.train_x_path)
            if self.save_preload and os.path.exists(self.train_x_path) and os.path.exists(self.test_x_path):
...
class QuantumEncodedMNISTOnDisk(QuantumEncodedMNIST):
    ...
    def preload_encoded_MNIST(self):
        ...
        print("here")
        print("train_val_dir", self.train_val_dir)
        print("test_dir", self.test_dir)
        print("path", os.path.join(self.train_val_dir, f"{0}.pt"))
        print("path2", os.path.join(self.test_dir, f"{0}.pt"))
...
def mnist_dataset_preencoded(params, mnist_path, path):
    return QuantumEncodedMNISTOnDisk(
